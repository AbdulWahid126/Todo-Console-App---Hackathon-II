...
**Network Flow**
1. User browser → `localhost:3000` (Next.js serves UI)
2. Client-side fetch → `localhost:3000/api/*` (Next.js route)
3. Next.js API route → `localhost:8000/todos` (FastAPI backend)
4. FastAPI → SQLite database file
...
├── todos/
│   ├── page.tsx            # Todo list view (Client Component)
│   ├── new/
│   │   └── page.tsx        # Create todo form (Client Component)
│   └── [id]/
│       └── edit/
│           └── page.tsx    # Edit todo form (Client Component)
├── components/
│   ├── TodoList.tsx        # List rendering logic
│   ├── TodoItem.tsx        # Single todo display
│   ├── TodoForm.tsx        # Reusable form (create/edit)
...
- `/todos/[id]/edit` → Edit specific todo
...
1. User submits form → Optimistically update local state
2. Send API request → Set loading state
3. On success → Navigate to list page (fresh fetch)
4. On failure → Revert optimistic update, show error
...
HTTP Request
    ↓
Router (todos.py) ────→ Validates input (Pydantic)
    ↓
Service Layer ─────────→ Business logic
    ↓
ORM Model ─────────────→ Database operations
    ↓
SQLite Database
```
...
- Ensures proper session lifecycle (open → use → close)
...
- **title**: Variable-length string
  - Stored trimmed (whitespace removed from start/end)
...
- **completed**: Boolean flag
  ...
  - No "archived" or "deleted" state (hard deletes)
...
**2xx Success Codes**
- **200 OK**: GET (single/list), PUT (update)
- **201 Created**: POST (create)
- **204 No Content**: DELETE

**4xx Client Error Codes**
- **400 Bad Request**: Validation failure (empty title, too long)
...
User submits form
    ↓
POST /todos → fetch() throws NetworkError
    ↓
...
Tab A: User edits to "Title A", submits
    ↓
Backend updates database (title → "Title A")
    ↓
Tab A refreshes, sees "Title A"
    ↓
Tab B: User (unaware) edits to "Title B", submits
    ↓
Backend updates database (title → "Title B")
    ↓
...
- SQLite → PostgreSQL: Change connection string, schema compatible
...
- Future phases may add relationships (e.g., Todo → Tags)
...
- **Migration Path**: Change connection string, schema compatible
...
- **Rationale
: Simplicity, API is source of truth
...
- [x] Persist across sessions (SQLite database)
...
- **Next Document**: Implementation Plan (Task Breakdown)
```

## Response snapshot

Created comprehensive implementation plan for Phase II Full-Stack Web Todo Application with all required components: plan.md with architecture decisions, research.md with technology choices, data-model.md with entity definitions, contracts/api-contracts.md with API specifications, quickstart.md with setup instructions, and tasks.md with detailed implementation tasks. The plan follows constitution requirements for Next.js 16 with App Router, FastAPI 0.115+, SQLModel, and Neon PostgreSQL.

## Outcome

- ✅ Impact: Created complete implementation plan with all required architectural decisions
- 🧪 Tests: none
- 📁 Files: Created specs/phase-ii/plan.md, specs/phase-ii/research.md, specs/phase-ii/data-model.md, specs/phase-ii/contracts/api-contracts.md, specs/phase-ii/quickstart.md, specs/phase-ii/tasks.md
- 🔁 Next prompts: Proceed with implementation using the task list
